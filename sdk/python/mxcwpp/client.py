...
class Client:
    ...
    def __init__(
        ...
        timeout: float = 30.0,
        user_agent: str = "mxcwpp-python-sdk/0.1.0",
    ):
    ...
    def _do(
        ...
        try:
            ...
            raise MxcwppError(f"url error: {e.reason}")

        try:
            j = json.loads(raw)
        except Exception:
            raise MxcwppError(f"non-json response: {raw[:200]}")
        ...
        if ar.code != 0:
            raise MxcwppError(ar.message, code=ar.code, status_code=status)
        return ar
