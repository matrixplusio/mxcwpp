// Package mxcwpp is the official Go SDK for mxcwpp platform API (P4-3).
//
// 用法:
//
//	cli, err := mxcwpp.NewClient("https://mxcwpp.example.com",
//	    mxcwpp.WithToken("eyJhbGc..."))
//	hosts, err := cli.ListHosts(ctx, mxcwpp.HostListOptions{Status: "online"})
//
...
func (c *Client) do(ctx context.Context, method, urlStr string, body any) (*ApiResponse, error) {
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("mxcwpp %s %d: %s", method, resp.StatusCode, string(raw))
	}
	var ar ApiResponse
	...
	if ar.Code != 0 {
		return nil, fmt.Errorf("mxcwpp api code=%d msg=%s", ar.Code, ar.Message)
	}
	return &ar, nil
}
