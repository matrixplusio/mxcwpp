...
func main() {
	...
	// MySQL 侧保留期清理：retention_policies 的天数只会下发成 ClickHouse TTL，
	// 而 data_source 默认全是 mysql，写在 MySQL 的数据此前无人清理。
	go managerscheduler.StartMySQLRetentionScheduler(services.DB, services.Logger)
