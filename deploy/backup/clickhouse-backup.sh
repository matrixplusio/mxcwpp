#!/usr/bin/env bash
# mxsec ClickHouse 备份脚本 (BACKUP DATABASE 内置命令)
#
# 调用:
#   ./clickhouse-backup.sh
#
# 备份 mxsec 库全表到 BACKUP_DIR + 上传 OSS。
# CH 用 BACKUP TO Disk('backups',...) 内置命令, 不阻塞读写。

set -euo pipefail

BACKUP_DIR="${BACKUP_DIR:-/var/lib/mxsec-backup/clickhouse}"
...
LOG_FILE="${LOG_FILE:-/var/log/mxsec-backup/clickhouse.log}"
