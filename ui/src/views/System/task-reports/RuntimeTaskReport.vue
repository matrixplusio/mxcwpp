      ...
      <div ref="reportContent" class="report-container">
        ...
        <div class="report-page">
          ...
          <div class="executive-summary">
            <div class="conclusion-banner" :class="report.summary.hasCriticalAlert ? 'critical' : report.summary.hasHighAlert ? 'high' : report.statistics.totalAlerts > 0 ? 'warning' : 'success'">
              <div class="conclusion-icon">
                <CheckCircleOutlined v-if="report.statistics.totalAlerts === 0" />
                <ExclamationCircleOutlined v-else />
              </div>
              <div class="conclusion-text">{{ report.summary.overallConclusion }}</div>
            </div>
            <div class="summary-content">
              <p class="summary-paragraph">{{ report.summary.alertOverview }}</p>
              <div class="summary-stats">
                <div class="stat-item">
                  <div class="stat-value" style="color: #F53F3F">{{ report.statistics.totalAlerts }}</div>
                  <div class="stat-label">总告警</div>
                </div>
                <div class="stat-item">
                  <div class="stat-value" style="color: #FF7D00">{{ report.statistics.activeAlerts }}</div>
        ...
        <div class="report-page">
          ...
          <table class="info-table">
            <tbody>
              ...
              <tr>
                <td class="label-cell">今日告警</td>
                <td class="value-cell">{{ report.statistics.todayAlerts }} 条</td>
                <td class="label-cell">已解决</td>
                <td class="value-cell">{{ report.statistics.resolvedAlerts }} 条</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="report-page">
          <div class="section-header">
            ...
            <div class="section-subtitle">Alert Statistics</div>
          </div>
          <div class="stats-grid">
            <div class="stats-card" style="border-left: 4px solid #F53F3F">
              <div class="stats-info">
                <div class="stats-value" style="color: #F53F3F">{{ report.statistics.totalAlerts }}</div>
                <div class="stats-label">总告警</div>
              </div>
            </div>
            <div class="stats-card" style="border-left: 4px solid #FF7D00">
              <div class="stats-info">
                <div class="stats-value" style="color: #FF7D00">{{ report.statistics.activeAlerts }}</div>
                <div class="stats-label">活跃</div>
              </div>
            </div>
            <div class="stats-card" style="border-left: 4px solid #00B42A">
              <div class="stats-info">
                <div class="stats-value" style="color: #00B42A">{{ report.statistics.resolvedAlerts }}</div>
          ...
          <div class="severity-distribution" v-if="report.statistics.totalAlerts > 0">
            <h4>按严重级别分布</h4>
            <div class="severity-bars">
              <div class="severity-bar-item" v-for="(count, severity) in report.statistics.bySeverity" :key="severity">
                ...
                <div class="severity-bar-container">
                  <div class="severity-bar" :class="severity" :style="{ width: `${(count / report.statistics.totalAlerts) * 100}%` }"></div>
                </div>
        ...
        <!-- 5. 规则分类分布 + MITRE ATT&CK -->
        <div class="report-page" v-if="report.statistics.byCategory.length > 0 || report.statistics.byMitre.length > 0">
          <div class="section-header">
            <div class="section-number">4</div>
            <div class="section-title">检测分类分布</div>
            <div class="section-subtitle">Detection Category Distribution</div>
          </div>
          <div class="severity-distribution" v-if="report.statistics.byCategory.length > 0">
            <h4>规则分类</h4>
            <div class="severity-bars">
              <div class="severity-bar-item" v-for="item in report.statistics.byCategory" :key="item.category">
                <div class="severity-label">
                  <span class="severity-tag medium">{{ item.category }}</span>
                </div>
                <div class="severity-bar-container">
                  <div class="severity-bar medium" :style="{ width: `${(item.count / report.statistics.totalAlerts) * 100}%` }"></div>
                </div>
                <div class="severity-count">{{ item.count }}</div>
              </div>
            </div>
          </div>
          <div class="severity-distribution" v-if="report.statistics.byMitre.length > 0" style="margin-top: 20px">
            <h4>MITRE ATT&CK 分布</h4>
            <div class="severity-bars">
              <div class="severity-bar-item" v-for="item in report.statistics.byMitre" :key="item.mitreId">
                <div class="severity-label">
                  <span class="severity-tag high">{{ item.mitreId }}</span>
                </div>
                <div class="severity-bar-container">
                  <div class="severity-bar high" :style="{ width: `${(item.count / report.statistics.totalAlerts) * 100}%` }"></div>
                </div>
                <div class="severity-count">{{ item.count }}</div>
              </div>
            </div>
        ...
        <div class="report-page" v-if="report.hostDetails.length > 0">
          <div class="section-header">
            <div class="section-number">{{ hasDistribution() ? '5' : '4' }}</div>
            <div class="section-title">主机告警明细</div>
            <div class="section-subtitle">Host Alert Details</div>
          </div>
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="host in report.hostDetails" :key="host.hostId">
                ...
                <td>{{ host.ip || '-' }}</td>
                <td><span style="color: #F53F3F">{{ host.alertCount }}</span></td>
        ...
        <!-- 7. Top 检测规则 -->
        <div class="report-page" v-if="report.topRules.length > 0">
          <div class="section-header">
            <div class="section-number">{{ getTopRulesSection() }}</div>
            <div class="section-title">Top 检测规则</div>
            <div class="section-subtitle">Top Detection Rules</div>
          </div>
          <table class="data-table">
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="rule in report.topRules" :key="rule.ruleId">
                <td>{{ rule.ruleId }}</td>
                <td>{{ rule.ruleName }}</td>
                <td><span class="severity-tag" :class="rule.severity">{{ getSeverityLabel(rule.severity) }}</span></td>
                <td>{{ rule.count }}</td>
              </tr>
            </tbody>
          </table>
...
const hasDistribution = (): boolean => {
  ...
  return report.value.statistics.byCategory.length > 0 || report.value.statistics.byMitre.length > 0
}

const getTopRulesSection = (): string => {
  if (!report.value) return '6'
  let n = 4
  if (hasDistribution()) n++
...
const getLastSection = (): string => {
  ...
  if (hasDistribution()) n++
  if (report.value.hostDetails.length > 0) n++
  if (report.value.topRules.length > 0) n++
  return String(n)
}
