...
# 生成物不入库（见 .gitignore），所以新克隆的仓库里这两个目录并不存在，
# 而 protoc 的 --go_out 不会自行创建它们——缺了这一步，任何人 clone 后
# 第一次 make proto 都会失败，且报错只说"No such file or directory"。
mkdir -p "${OUT_DIR}/bridge" "${OUT_DIR}/grpc"
