...
MxSec Platform 采用 **Agent + Plugin + 六服务后端** 分层架构。后端拆分为 **Manager / AgentCenter / Consumer / Engine / LLMProxy / VulnSync** 六个独立服务，每个服务独立部署、独立扩缩容。控制面无状态，支持多实例水平扩展；数据面通过 Kafka 异步解耦，按存储特征分层写入 MySQL（业务主数据）和 ClickHouse（时序与事件归档）。
...
```
      ...
      +--------------------+   +-------+------------+   +-------+------------+
                                       ^                        |
                                       | Kafka mxsec.agent.*    | Kafka mxsec.vuln.advisory
                                       |                        v
      +--------------------+   +-------+------------+   +--------------------+
...
- 订阅 8 个业务 Topic（ConsumerGroup A "mxsec-consumer"），按 DataType 路由写入 MySQL / ClickHouse
...
- 订阅 `mxsec.agent.*` 全部业务 Topic（ConsumerGroup B "mxsec-engine"），与 Consumer 互不影响 offset
...
- 多层检测子包：
  ...
  - `rollout/` — 规则灰度发布
- 产出：`mxsec.engine.alert` / `mxsec.engine.storyline` / `mxsec.engine.feedback`
...
- 审计：所有调用入 Kafka `mxsec.llm.audit`

入口：`cmd/server/llmproxy/main.go`

### VulnSync

v2.0 新增的漏洞情报融合服务，定时聚合多源漏洞数据。

- **11+ 外部源**：NVD / OSV / RHSA / USN / Debian / Alpine / SUSE / CISA KEV / ExploitDB / CNNVD / EPSS + 信创 4 源
- PURL+NEVRA 双索引模型 + 3 级 confidence 仲裁
- 推送 advisory 到 Kafka `mxsec.vuln.advisory`，Manager 与 Engine 消费匹配主机漏洞
- Leader Election（redsync）避免多副本重复抓取
...
Plugin --> Agent Ring Buffer --> gRPC(mTLS) --> AgentCenter
    ...
    ├── ConsumerGroup A "mxsec-consumer" --> MySQL / ClickHouse / Redis（持久化）
    └── ConsumerGroup B "mxsec-engine"  --> 16 Stage 流水线（检测）
