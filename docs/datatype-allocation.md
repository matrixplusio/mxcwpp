...
| 6000 | Server→Plugin | FIM 检查任务下发 | AC TaskService | FIM 插件 |
| 6003 | Server→Plugin | FIM 基线回写（确认变更后） | Manager FIMBaselinePusher | FIM 插件 `handleBaselineDelivery` |
| 6005-6099 | - | **未分配** | - | - |

> **6004 特殊路径**: AC 在 Kafka 发送前拦截 6004，直接调用 `handleFIMBaselineSnapshot()`。
>
> **6003 曾长期只有接收方**：Agent 侧 `handleBaselineDelivery` 自始存在，服务端发送方一直没写。
> 后果是 FIM 基线只在首扫落地，此后每轮都拿首扫快照做基准，凡变化过一次的文件永远判
> `changed`——控制台「确认变更」点了不报错，基线却不动。一次系统包升级因此累积出
> 22,588 条永不收敛的告警。发送方已补（`biz.FIMBaselinePusher`）。
...
>
> **已加 `make test` 闸门**: `internal/server/common/kafka/routing_doc_test.go` 双向比对本表与
