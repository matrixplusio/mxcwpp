...
- ✅: 集群内镜像扫描(trivy-operator 全生命周期: 预检/双路径安装/状态机/Pull+Push 同步/卸载, GKE UAT e2e 通 — 见 docs/cluster-image-scan-design.md、docs/gke-onboarding.md)
- ✅: GKE 审计检测 GCP Pub/Sub 链路真验通(8452 事件) + K8S-006 误报修复(弃 UA 白名单→公网IP/人类客户端) + 规则 8→14(port-forward/attach/ephemeral/删审计/匿名/webhook篡改) + detector 确定性单测 — 见 docs/gke-audit-detection.md
- ✅: 中心镜像扫描重构为独立 scanner 微服务(scan_jobs 队列 + trivy,manager 只入队)+ Registry 仓库接入(type/GCR-GAR-ACR 认证 + 仓库管理 UI)+ 镜像 CVE 富化关联统一漏洞库(EPSS/KEV/CNNVD/信创)— 见 docs/image-scan-extension-design.md;剩 S3 定时重扫/S4 CI 门禁
- ✅: 集群详情页(/kube/clusters/detail:概览/节点/Pod/工作负载/GCP审计配置/集群内扫描器)+ 多集群筛选(告警/事件/基线)+ GCP Pub/Sub 配置 UI(补回重构丢失入口)
- ✅: 基线检查改任务为指向(kube_baseline_tasks + task_id,任务列表→点开看该次 checklist,留最近 10 任务)
- ⚪: 微隔离 Phase1/2/3、imagescan secret/dockerfile/license、Admission Handler(见风险清单)、镜像扫描 S3 定时重扫/S4 CI门禁
- 🟢: Helm server+agent chart
- 🟡: Docker CIS 29 条、K8s CIS 80 条(目标 120)、License 扫描、KMS+KubeConfig 集成
...
- ✅: 主机基线任务详情独立页(/baseline/tasks/detail?id=,静态导出兼容)— 检查项按规则聚合多主机,逐项看 说明/检查依据/不通过主机(受影响资源+实际值)/修复建议;新增 GET /tasks/:id/checks 聚合端点(任务"失败"与合规"不通过"严格区分)
