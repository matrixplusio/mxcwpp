...
func setupZombieDB(t *testing.T) *gorm.DB {
	...
	tables := []string{
		...
		// 明细表：任务超时后未回报主机要在这里被标成 timeout。
		// 缺这张表的话新逻辑会静默不生效，而那正是要守住的行为。
...
// TestTimeoutZombieTasks_FIMTaskPartial 超时的 FIM 任务按回报情况分级。
//
// 任务只在 completed_host_count 追平 dispatched_host_count 时才转 completed，
// 所以一台主机不回报就会把整批拖到超时。修复前这种情况一律记 failed，
// 使得"198/200 台成功"和"一台都没跑起来"在任务列表上完全一样。
func TestTimeoutZombieTasks_FIMTaskPartial(t *testing.T) {
	db := setupZombieDB(t)
	s := newTestScheduler(db)

	oldTime := time.Now().Add(-3 * time.Hour)

	// 只差两台没回报——其余主机的扫描结果都已落库
	db.Exec(`INSERT INTO fim_tasks (task_id, status, dispatched_host_count, completed_host_count, updated_at)
	         VALUES (?, ?, ?, ?, ?)`, "fim-partial-1", "running", 200, 198, oldTime)
	// 一台都没回报——任务确实没跑起来
	db.Exec(`INSERT INTO fim_tasks (task_id, status, dispatched_host_count, completed_host_count, updated_at)
	         VALUES (?, ?, ?, ?, ?)`, "fim-nothing-1", "running", 200, 0, oldTime)

	s.timeoutZombieTasks()

	var partial model.FIMTask
	db.Where("task_id = ?", "fim-partial-1").First(&partial)
	if partial.Status != "partial" {
		t.Errorf("有主机回报的超时任务应判 partial, got %s", partial.Status)
	}
	if partial.CompletedAt == nil {
		t.Error("partial 任务也应写入 completed_at，否则会被反复扫描")
	}

	var nothing model.FIMTask
	db.Where("task_id = ?", "fim-nothing-1").First(&nothing)
	if nothing.Status != "failed" {
		t.Errorf("无任何主机回报的超时任务应判 failed, got %s", nothing.Status)
	}
}

// TestTimeoutZombieTasks_FIMHostStatusConverges 任务超时后，明细表里未回报的
// 主机要从 dispatched 收敛为 timeout。
//
// dispatched 读起来像"刚下发、还在执行"，但任务已经放弃等待、不会再有回报。
// 状态不对齐时，任务详情里列出的未回报主机全是 dispatched，
// 看不出到底是执行中还是已放弃。已回报的记录不受影响。
func TestTimeoutZombieTasks_FIMHostStatusConverges(t *testing.T) {
	db := setupZombieDB(t)
	s := newTestScheduler(db)
	...
	db.Exec(`INSERT INTO fim_tasks (task_id, status, dispatched_host_count, completed_host_count, updated_at)
	         VALUES (?, ?, ?, ?, ?)`, "fim-conv-1", "running", 3, 1, oldTime)

	db.Exec(`INSERT INTO fim_task_host_status (task_id, host_id, status) VALUES (?, ?, ?)`,
		"fim-conv-1", "h-done", "completed")
	db.Exec(`INSERT INTO fim_task_host_status (task_id, host_id, status) VALUES (?, ?, ?)`,
		"fim-conv-1", "h-silent-1", "dispatched")
	db.Exec(`INSERT INTO fim_task_host_status (task_id, host_id, status) VALUES (?, ?, ?)`,
		"fim-conv-1", "h-silent-2", "dispatched")

	// 未超时的任务，其明细不应被动到
	db.Exec(`INSERT INTO fim_tasks (task_id, status, dispatched_host_count, completed_host_count, updated_at)
	         VALUES (?, ?, ?, ?, ?)`, "fim-fresh-2", "running", 2, 1, time.Now())
	db.Exec(`INSERT INTO fim_task_host_status (task_id, host_id, status) VALUES (?, ?, ?)`,
		"fim-fresh-2", "h-running", "dispatched")

	s.timeoutZombieTasks()

	var timedOut int64
	db.Raw(`SELECT COUNT(*) FROM fim_task_host_status WHERE task_id='fim-conv-1' AND status='timeout'`).Scan(&timedOut)
	if timedOut != 2 {
		t.Errorf("两台未回报主机应收敛为 timeout, got %d", timedOut)
	}

	var stillCompleted int64
	db.Raw(`SELECT COUNT(*) FROM fim_task_host_status WHERE task_id='fim-conv-1' AND status='completed'`).Scan(&stillCompleted)
	if stillCompleted != 1 {
		t.Errorf("已回报的记录不应被改写, got %d", stillCompleted)
	}

	var freshUntouched int64
	db.Raw(`SELECT COUNT(*) FROM fim_task_host_status WHERE task_id='fim-fresh-2' AND status='dispatched'`).Scan(&freshUntouched)
	if freshUntouched != 1 {
		t.Errorf("未超时任务的明细不应被改写, got %d", freshUntouched)
	}
}
