...
//
//	mxcwpp_vuln_total{source,confidence,severity}  — 当前漏洞总数
//	mxcwpp_remediation_task_total{status,severity} — 修复任务计数
//	mxcwpp_remediation_task_duration_seconds       — 修复耗时分布
//	mxcwpp_vuln_import_errors_total{source}        — 数据源拉取错误数
//	mxcwpp_vuln_integrity_check_failures_total     — 数据完整性反查失败数
package biz
...
var VulnMetrics = struct {
	...
	RemediationTaskTotal: promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "mxcwpp_remediation_task_total",
	...
	RemediationTaskDuration: promauto.NewHistogramVec(prometheus.HistogramOpts{
		Name:    "mxcwpp_remediation_task_duration_seconds",
...
//	  expr: rate(mxcwpp_vuln_import_errors_total[5m]) > 0.1
//	  for: 10m
//	  annotations:
...
//	  expr: |
//	    sum(rate(mxcwpp_remediation_task_total{status="completed"}[1h]))
//	      / sum(rate(mxcwpp_remediation_task_total[1h])) < 0.95
//	  for: 30m
//	  annotations:
//	    summary: "1h 修复成功率 < 95%（商业级 SLA 阈值）"
//
//	- alert: VulnDataIntegrityFailing
//	  expr: increase(mxcwpp_vuln_integrity_check_failures_total[24h]) > 1
//	  annotations:
