...
func TestCreateScanTask_BusinessLineResolve(t *testing.T) {
	...
	for _, h := range []model.Host{
		{HostID: "host-1", BusinessLine: "line-a"},
		{HostID: "host-2", BusinessLine: "line-a"},
		{HostID: "host-3", BusinessLine: "line-b"},
	} {
	...
	require.NoError(t, err)
	assert.Equal(t, "line-a", task.BusinessLine)
	assert.Equal(t, 2, task.ProgressTotal, "应解析出 2 台 line-a 主机")
}
