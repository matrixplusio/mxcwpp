...
//
// 落库通道由 audit.Init 统一决定（feature_flag.data_source.audit_log）。
func AuditLog(db *gorm.DB, logger *zap.Logger) gin.HandlerFunc {
	return AuditLogWithCH(db, nil, logger)
}

// AuditLogWithCH 为兼容旧签名保留 chConn 参数；实际写入走 audit 全局记录器。
func AuditLogWithCH(_ *gorm.DB, _ chdriver.Conn, _ *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		audit.Record(c.Request.Context(), audit.Event{
			...
			Action:       audit.SemanticAction(method, c.FullPath(), resourceType),
