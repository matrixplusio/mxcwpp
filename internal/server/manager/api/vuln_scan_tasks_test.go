...
func TestTriggerScan_ScopeHosts_Returns200(t *testing.T) {
	...
	require.NoError(t, db.Create(&model.Host{HostID: "host-1", BusinessLine: "line-a"}).Error)
...
func TestTriggerScan_ScopeBusinessLine_ResolvesHosts(t *testing.T) {
	...
	for _, host := range []model.Host{
		{HostID: "h-1", BusinessLine: "line-a"},
		{HostID: "h-2", BusinessLine: "line-a"},
		{HostID: "h-3", BusinessLine: "line-b"},
	} {
