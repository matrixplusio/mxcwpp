...
type FIMEventsHandler struct {
	...
	logger *zap.Logger
	// pusher 为 nil 时确认仍然成功，只是基线不回写——单机部署或未接 AC 时是这样。
	pusher *biz.FIMBaselinePusher
}
...
// WithBaselinePusher 接入基线回写能力。
func (h *FIMEventsHandler) WithBaselinePusher(p *biz.FIMBaselinePusher) *FIMEventsHandler {
	h.pusher = p
	return h
}
...
func (h *FIMEventsHandler) ConfirmFIMEvent(c *gin.Context) {
	...
	// 回写基线。不写回的话，同一文件下一轮扫描仍与首扫快照比对，永远判变更——
	// 「确认」按钮在 UI 上有、点了不报错，但基线不动，这正是此前的行为。
	msg := "事件已确认"
	if req.UpdateBaseline {
		switch {
		case h.pusher == nil:
			msg = "事件已确认；基线回写未启用，该文件下轮扫描仍会告警"
			h.logger.Warn("请求回写基线但 pusher 未接线", zap.String("event_id", eventID))
		default:
			if err := h.pusher.PushForConfirmedEvent(&event); err != nil {
				// 确认本身已落库，不因回写失败回滚：确认是人的判断，
				// 回写是执行动作，后者失败不该抹掉前者。
				msg = "事件已确认；基线回写失败，该文件下轮扫描仍会告警"
				h.logger.Error("回写 FIM 基线失败",
