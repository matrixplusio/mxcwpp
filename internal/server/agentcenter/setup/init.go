...
// schedulerVerifyDelay 是后台调度器自检前的宽限期。
//
// 每个调度器都是 go 出去的，主流程走到自检时它们未必已执行到 MarkStarted。
// 取值只需覆盖 goroutine 首次被调度的时间，3 秒远超所需，且自检本身也在
// 独立 goroutine 里，不阻塞启动。
const schedulerVerifyDelay = 3 * time.Second
...
func (s *AgentCenterServices) StartBackgroundServices() {
	...
	// 启动 IOC 同步调度器（广播威胁情报到 Agent 内置 EDR）。
	//
	// 此前这行只存在于 agentcenter/init/init.go —— 一份没有任何调用方的文件。
	// AC 主程序走的是本文件的 Initialize，于是 agent 侧的 IOC 匹配自上线起
	// 就没拿到过任何情报：全机群 edr_ioc_count 为 0。
	iocScheduler := scheduler.NewIOCSyncScheduler(s.DB, s.TransferService, s.Logger)
	go iocScheduler.Start(s.StatusCtx)

	// 连接即推：新连/重连的 agent 立即收到全量 IOC。
	// 只靠 5 分钟周期广播不够——agent 重启后到下次广播之间，本地 IOC 集是空的，
	// 而重启恰恰是最需要立即恢复检测能力的时刻。
	s.TransferService.OnAgentConnect(iocScheduler.PushToAgent)

	// 启动规则同步调度器（广播 Agent 检测规则）。
	//
	// 同样此前只在死代码里。后果是 agent 规则完全无法热更新：agent 上的规则
	// 来自二进制内嵌资源在首次启动时的释放，而 installBuiltinRules 对已存在的
	// 文件直接跳过，所以连重装 agent 都不会更新规则。
	ruleScheduler := scheduler.NewRuleSyncScheduler(s.DB, s.TransferService, s.Logger)
	go ruleScheduler.Start(s.StatusCtx)

	// 自检：确认上面每一个 go 出去的调度器都真的跑起来了。
	//
	// 漏接线不会有任何报错——这正是 ioc_sync 与 rule_sync 能静默缺失数月的原因。
	// 每个调度器在自身 Start 里登记，这里比对期望与实际，缺失即 ERROR + 指标非零。
	// 延迟一小会儿再比对，给 goroutine 调度留出时间，否则必然误报。
	go func() {
		time.Sleep(schedulerVerifyDelay)
		scheduler.VerifyStarted(s.Logger)
	}()
}
