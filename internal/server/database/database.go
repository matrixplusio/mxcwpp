...
// GORM 日志写入的两道闸门。
//
// GORM 报错时会把完整 SQL 语句原样交给 Writer。批量 INSERT 的语句可达数 KB，
// 而失败往往是成片的：2026-08 alerts 表写满后，每一条失败重试都打印一条含完整
// INSERT 的日志，单日写出 20GB，最终写满 node1 根盘让平台停摆。
//
// 截断保证单条有上限，限速保证总量有上限。两者都必要：只截断挡不住高频，
// 只限速挡不住单条超长。被丢弃的条数会在下一条放行的日志里带出来，
// 不会让"日志变少了"看起来像"问题消失了"。
const (
	gormLogMaxBytes   = 1024 // 单条日志最大字节数，超出截断
	gormLogMaxPerSec  = 20   // 每秒放行条数
	gormLogWindowSpan = time.Second
)
...
// emit 施加截断与限速后写日志。
func (w *zapWriter) emit(msg string) {
	if len(msg) > gormLogMaxBytes {
		msg = msg[:gormLogMaxBytes] + fmt.Sprintf("...(截断，原长 %d 字节)", len(msg))
	}

	w.mu.Lock()
	now := time.Now()
	if now.Sub(w.windowStart) >= gormLogWindowSpan {
		w.windowStart = now
		w.windowCount = 0
	}
	w.windowCount++
	if w.windowCount > gormLogMaxPerSec {
		w.suppressed++
		w.mu.Unlock()
		return
	}
	...
	if dropped > 0 {
		w.logger.Warn(msg, zap.Int64("suppressed_since_last", dropped))
		return
	}
	w.logger.Info(msg)
}
