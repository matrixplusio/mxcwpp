...
func (w *MySQLWriter) WriteFIMTaskComplete(msg *kafka.MQMessage) error {
	...
	return w.db.Transaction(func(tx *gorm.DB) error {
		...
		// 2. 递增 FIMTask.completed_host_count，并累加本机发现的变更数。
		//    total_events 此前从未被写入，任务列表上永远显示 0，
		//    看不出一次扫描到底扫出了什么，只能回到明细表里数。
		if err := tx.Model(&model.FIMTask{}).
			Where("task_id = ?", taskID).
			Updates(map[string]interface{}{
				"completed_host_count": gorm.Expr("completed_host_count + 1"),
				"total_events": gorm.Expr("total_events + ?",
					addedCount+removedCount+changedCount),
			}).Error; err != nil {
			return fmt.Errorf("递增 FIMTask 计数失败: %w", err)
		}
...
// 实例情况下的重复写入。
//
// memfd_exec 严重度从 critical 降到 high — dbus/runc 类 memfd_exec 极常见,
