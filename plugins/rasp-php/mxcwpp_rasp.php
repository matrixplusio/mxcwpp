<?php
/**
 * mxcwpp PHP RASP — auto_prepend_file 模式 (P4-3).
 *
 * 启用 (无需重新编译 PHP):
 *
 *   ; /etc/php/8.x/fpm/php.ini
 *   auto_prepend_file = /opt/mxcwpp/rasp/mxcwpp_rasp.php
 *
 ...
 *   - 失败必须 silent (永不影响业务流)
 *
 * 完整 Zend extension (libmxcwpp_rasp_php.so) 留 Sprint 5+, 性能 5x+.
 */

namespace MxcwppRasp;

class Agent {
    private static $reporter = null;
    private static $config = [
        'uds_path' => '/var/run/mxcwpp-rasp.sock',
        'tenant_id' => 't-default',
        'enabled' => true,
    ];
    ...
    private static function captureStack($maxFrames = 30) {
        ...
        foreach ($stack as $frame) {
            if (isset($frame['class']) && strpos($frame['class'], 'MxcwppRasp\\') === 0) continue;
...
Agent::install([
    'uds_path' => getenv('MXCWPP_RASP_UDS') ?: '/var/run/mxcwpp-rasp.sock',
    'tenant_id' => getenv('MXCWPP_RASP_TENANT') ?: 't-default',
]);
 ...
 * 推荐: 编译 libmxcwpp_rasp_php.so Zend extension 走 zend_compile_string +
