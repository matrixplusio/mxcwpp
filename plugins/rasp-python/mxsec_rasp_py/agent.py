...
class RaspConfig:
    """RASP 配置."""
    uds_path: str = "/var/run/mxsec-rasp.sock"
    ...
    def from_env(cls) -> "RaspConfig":
        return cls(
            uds_path=os.environ.get("MXSEC_RASP_UDS", "/var/run/mxsec-rasp.sock"),
            tenant_id=os.environ.get("MXSEC_RASP_TENANT", "t-default"),
            enabled=os.environ.get("MXSEC_RASP_ENABLED", "1") != "0",
        )
...
def install(uds_path: Optional[str] = None,
    ...
    try:
        ...
        sys.stderr.write(f"[mxsec-rasp] addaudithook failed: {e}\n")
...
def _capture_stack(max_frames: int = 30) -> list:
    ...
    try:
        ...
        out = []
        for f in frames:
            if "mxsec_rasp_py" in f.filename:
                continue
